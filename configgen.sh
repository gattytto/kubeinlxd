...
yq eval "select (di == 1) .controlPlaneEndpoint = \"[1100:200::1]:6443\"" -i "$KUBEADM_CONFIG"
...
yq eval "select (di == 1) .scheduler.extraArgs.bind-address = \"1100:200::1\"" -i "$KUBEADM_CONFIG"
yq eval "select (di == 1) .controllerManager.extraArgs.bind-address = \"::\"" -i "$KUBEADM_CONFIG"
