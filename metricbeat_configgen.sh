...
yq eval "del(select(di == 9).spec.template.spec.containers[0].env[3].value)" -P -i metricbeat-kubernetes.yaml
yq eval "select(di == 9).spec.template.spec.containers[0].env[3].valueFrom.secretKeyRef = {\"name\":\"$SEC_KEY_REF\",\"key\":\"elastic\"}" -P -i metricbeat-kubernetes.yaml
